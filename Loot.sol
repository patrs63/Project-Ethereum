...
contract Loot {
    ...
    event addedLoot(uint256 id, uint256 data, address addr);
    ...
    struct Object{
        uint256 id;
        uint256 next;
        uint256 data; // the amount to send them to
        address addr; // the address to send the money to
    }
     ...
     */
    function addHead(uint256 _data, address _addr)
        public
    {
        uint256 objectId = _createObject(_data, _addr);
        _link(objectId, head);
        _setHead(objectId);
    }
     ...
     */
    function addTail(uint256 _data, address _addr) // NOTE THAT IT IS BETTER TO USED ADD TAIL FOR EFFICIENCY DURING DELETION
        public
    {
        if (head == 0) {
            addHead(_data, _addr);
            ...
            uint256 newTailId = _createObject(_data, _addr);
     ...
     */
    function _createObject(uint256 _data, address _addr)
        internal
        returns (uint256)
    {
        uint256 newId = idCounter;
        idCounter += 1;
        Object memory object = Object(newId, 0, _data, _addr);
